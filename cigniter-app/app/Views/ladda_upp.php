...
<form enctype="multipart/form-data" action="<?= base_url() ?>ladda_upp/upload" method="post" class="form" onSubmit="return form_check()">
    ...
    <div class="col2 clear">
        ...
        <input type="text" name="name" id="name" class="input" value="<?= isset($name) ? $name : '' ?>"/>

        <label for="comment">Kommentar</label>
        <textarea name="comment" id="comment" class="input" rows="5"/><?= isset($comment) ? $comment : false ?></textarea>

        <label for="order_id">Ordernummer</label>
        <input type="text" name="order_id" id="order_id" class="input" value="<?= isset($order_id) ? $order_id : '' ?>"/>

        <label for="done_before">Ska vara klart</label>
        <input type="text" name="done_before" id="done_before" class="input" value="<?= isset($done_before) ? $done_before : false ?>"/>
    </div>
    <div class="col2">
        <label for="theimage">
            Bild
